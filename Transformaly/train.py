...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr", default=0.0001, help="Learning rate value")
    parser.add_argument(
        "--eval_every",
        type=int,
        default=2,
    ...
    parser.add_argument(
        "--plot_every_layer_summarization",
        default=False,
        action="store_true",
    ...
    logging.basicConfig(
        filename=join(
            BASE_PATH,
            f'{"unimodal" if args["unimodal"] else "multimodal"}/{args["dataset"]}',
            f'Train_{args["dataset"]}_Transformaly_outputs.log',
        ),
        level=logging.DEBUG,
    )
    ...
    for _class in _classes:
        ...
        print_and_add_to_log("Start Training", logging)
        print_and_add_to_log(
            "====================================================================",
            logging,
        )
        ...
        _, ood_test_set = get_datasets_for_ViT(
            dataset=args["dataset"],
            data_path=args["data_path"],
        ...
        ood_val_loader = torch.utils.data.DataLoader(
            ood_test_set, batch_size=args["batch_size"], shuffle=False
        )
        ...
        model_checkpoint_path = join(
            model_path, "last_full_finetuned_model_state_dict.pkl"
        )
        ...
        model, best_model, cur_acc_loss = train(
            model=model,
        ...
        torch.save(
            ...
            join(model_path, "best_full_finetuned_model_state_dict.pkl"),
        )
        torch.save(
            model.state_dict(),
            join(model_path, "last_full_finetuned_model_state_dict.pkl"),
        )
