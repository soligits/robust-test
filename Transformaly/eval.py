...
from Transformaly.utils import get_finetuned_features_adversarial
import torch.nn
from utils import (
    print_and_add_to_log,
    get_datasets_for_ViT,
    Identity,
    get_finetuned_features,
)
from pytorch_pretrained_vit.model import AnomalyViT
import KnnAdvancedPGD

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="")
    parser.add_argument("--dataset", default="cifar10")
    parser.add_argument("--data_path", default="./data/", help="Path to the dataset")
    parser.add_argument("--attack_steps", type=int, default=10, help="Number of Steps")
    parser.add_argument("--attack_eps", default="2/255", help="Attack Epsilon")
    parser.add_argument(
        "--whitening_threshold",
        default=0.9,
        type=float,
    ...
    logging.basicConfig(
        filename=join(
            BASE_PATH,
            f'{"unimodal" if args["unimodal"] else "multimodal"}/{args["dataset"]}',
            f'Eval_{args["dataset"]}_Transformaly_outputs.log',
        ),
        level=logging.DEBUG,
    )
    ...
    for _class in _classes:
        ...
        trainset, testset = get_datasets_for_ViT(
        ...
        with open(
            join(
                BASE_PATH,
                f'{"unimodal" if args["unimodal"] else "multimodal"}',
        ...
        test_adv_features = np.ascontiguousarray(pca.transform(test_adv_features))
        print_and_add_to_log("Whitening ended", logging)

        # build GMM
        dens_model = mixture.GaussianMixture(
            n_components=1, max_iter=1000, verbose=1, n_init=1
        )
        dens_model.fit(train_features)
        test_pretrained_samples_likelihood = dens_model.score_samples(test_features)
        test_adv_pretrained_samples_likelihood = dens_model.score_samples(
            test_adv_features
        )
        print_and_add_to_log("----------------------", logging)

        pretrained_auc = roc_auc_score(
            anomaly_targets, test_pretrained_samples_likelihood
        )
        pretrained_adv_auc = roc_auc_score(
            anomaly_targets, test_adv_pretrained_samples_likelihood
        )

        print_and_add_to_log(f"Pretrained AUROC score is: {pretrained_auc}", logging)
        print_and_add_to_log(
            f"Pretrained Adversarial AUROC score is: {pretrained_adv_auc}", logging
        )
        print_and_add_to_log("----------------------", logging)
        ...
        if not os.path.exists(
            join(
                base_feature_path, "features_distances", FINETUNED_PREDICTION_FILE_NAME
            )
        ):
            print_and_add_to_log("Load Model", logging)
            model_checkpoint_path = join(
                model_path, "best_full_finetuned_model_state_dict.pkl"
            )
            ...
            test_finetuned_features = get_finetuned_features(
                model=model, loader=test_loader
            )
            test_adv_finetuned_features = get_finetuned_features_adversarial(
                model=model,
                loader=test_loader,
                attack=KnnAdvancedPGD.PGD_KNN_ADVANCED(
                    ...
                    k=2,
                    eps=eval(args["attack_eps"]),
                    steps=args["attack_steps"],
                    alpha=2.5 * eval(args["attack_eps"]) / args["attack_steps"],
                ),
            )
            ...
            np.save(
                join(
                    base_feature_path,
                    "features_distances",
                    "adv_" + FINETUNED_PREDICTION_FILE_NAME,
                ),
                test_adv_finetuned_features,
            )
            ...
            test_adv_finetuned_features = np.load(
                join(
                    base_feature_path,
                    "features_distances",
                    "adv_" + FINETUNED_PREDICTION_FILE_NAME,
        ...
        if test_adv_finetuned_features.shape[0] == 1:
            test_adv_finetuned_features = test_adv_finetuned_features[0]
        ...
        if not os.path.exists(
            join(
                base_feature_path, "features_distances", "train_finetuned_features.npy"
            )
        ):
            print_and_add_to_log("Load Model", logging)
            model_checkpoint_path = join(
            ...
            train_finetuned_features = get_finetuned_features(
            ...
            train_finetuned_features = np.load(
                join(
                    base_feature_path,
                    "features_distances",
                    "train_finetuned_features.npy",
        ...
        train_finetuned_features = train_finetuned_features[
            :, args["use_layer_outputs"]
        ]

        test_finetuned_features = test_finetuned_features[:, args["use_layer_outputs"]]
        test_adv_finetuned_features = test_adv_finetuned_features[
            :, args["use_layer_outputs"]
        ]

        gmm_scores = []
        train_gmm_scores = []
        gmm = mixture.GaussianMixture(n_components=1, max_iter=500, verbose=1, n_init=1)
        gmm.fit(train_finetuned_features)
        test_finetuned_samples_likelihood = gmm.score_samples(test_finetuned_features)
        test_adv_finetuned_samples_likelihood = gmm.score_samples(
            test_adv_finetuned_features
        )
        ...
        test_finetuned_auc = roc_auc_score(
            anomaly_targets, test_finetuned_samples_likelihood
        )
        test_adv_finetuned_auc = roc_auc_score(
            anomaly_targets, test_adv_finetuned_samples_likelihood
        )
        print_and_add_to_log(
            f"All Block outputs prediciton AUROC score is: {test_finetuned_auc}",
            logging,
        )
        print_and_add_to_log(
            f"All Block outputs prediciton AUROC Adversarial score is: {test_adv_finetuned_auc}",
            logging,
        )
        ...
        finetuned_and_pretrained_samples_likelihood = [
            test_finetuned_samples_likelihood[i] + test_pretrained_samples_likelihood[i]
            for i in range(len(test_pretrained_samples_likelihood))
        ]

        finetuned_and_pretrained_samples_likelihood_adv = [
            test_adv_finetuned_samples_likelihood[i]
            + test_adv_pretrained_samples_likelihood[i]
            for i in range(len(test_adv_pretrained_samples_likelihood))
        ]

        finetuned_and_pretrained_auc = roc_auc_score(
            anomaly_targets, finetuned_and_pretrained_samples_likelihood
        )

        finetuned_and_pretrained_auc_adv = roc_auc_score(
            anomaly_targets, finetuned_and_pretrained_samples_likelihood_adv
        )
        ...
        print_and_add_to_log(
            f"The bgm and output prediction prediciton Adversarial AUROC is: {finetuned_and_pretrained_auc_adv}",
            logging,
        )

        results["pretrained_and_finetuned_AUROC_scores"].append(
            finetuned_and_pretrained_auc
        )

    results_pd = pd.DataFrame.from_dict(results)
    results_dict_path = join(
        BASE_PATH, f'summarize_results/{args["dataset"]}/{args["dataset"]}_results.csv'
    )
